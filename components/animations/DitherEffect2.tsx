...
class ShaderMountVanilla {
  ...
  constructor(
    ...
    // Explicit immediate initial dimension measurement
    const rect = this.parentElement.getBoundingClientRect();
    this.parentWidth = rect.width || (typeof window !== "undefined" ? window.innerWidth : 1920);
    this.parentHeight = rect.height || (typeof window !== "undefined" ? window.innerHeight : 1080);
    this.handleResize();
  ...
  handleResize = () => {
    if (!this.parentWidth || this.parentWidth <= 0) {
      this.parentWidth = typeof window !== "undefined" ? window.innerWidth : 1920;
    }
    if (!this.parentHeight || this.parentHeight <= 0) {
      this.parentHeight = typeof window !== "undefined" ? window.innerHeight : 1080;
    }

    let targetPixelWidth = 0;
    let targetPixelHeight = 0;
    const dpr = typeof window !== "undefined" ? Math.max(1, window.devicePixelRatio) : 1;
    const pinchZoom = (typeof window !== "undefined" && window.visualViewport?.scale) ? window.visualViewport.scale : 1;
    if (this.devicePixelsSupported && this.parentDevicePixelWidth > 0) {
...
function bestGuessBrowserZoom(doc: Document) {
  const viewportScale = typeof window !== "undefined" && window.visualViewport ? window.visualViewport.scale : 1;
  const viewportWidth = typeof window !== "undefined" && window.visualViewport ? window.visualViewport.width : (typeof window !== "undefined" ? window.innerWidth : 1200);
  const scrollbarWidth = typeof window !== "undefined" ? window.innerWidth - doc.documentElement.clientWidth : 0;
  const innerWidth = viewportScale * viewportWidth + scrollbarWidth;
  const ratio = (typeof window !== "undefined" ? window.outerWidth : 1200) / innerWidth;
...
export const DitheringShapes: Record<string, number> = {
  simplex: 1,
  warp: 2,
  dots: 3,
  wave: 4,
...
export const DitheringTypes: Record<string, number> = { random: 1, "2x2": 2, "4x4": 3, "8x8": 4 };
...
export const ShaderMount = forwardRef<HTMLDivElement, ShaderMountProps>(function ShaderMountImpl(
  {
    fragmentShader,
  ...
  useEffect(() => {
    if (divRef.current && !shaderMountRef.current) {
      try {
        ...
        console.warn("ShaderMount initialization:", err);
      }
    }
    return () => {
...
export interface DitheringProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "color"> {
  speed?: number;
  frame?: number;
  colorBack?: string;
  colorFront?: string;
...
export const Dithering = memo(function DitheringImpl({
  speed = 1,
  frame = 0,
  colorBack = "#000000",
  colorFront = "#00FFC0",
...
export interface DitherEffect2Props {
  background?: string;
  color?: string;
  size?: number;
  speed?: number;
...
export default function DitherEffect2(props: DitherEffect2Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const S = settingsFor(props);

  // Always keep speed active for background cover
  const effectiveSpeed = S.speed;
