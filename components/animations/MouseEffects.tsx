...
export function MouseEffects({
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      const id = `${e.timeStamp}-${Math.round(x)}-${Math.round(y)}-${Math.random().toString(36).substring(2, 6)}`;
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden="true"
      className="fixed inset-0 w-full h-full pointer-events-none z-[9999] overflow-hidden select-none"
      style={{
        position: "fixed",
        inset: 0,
        width: "100vw",
