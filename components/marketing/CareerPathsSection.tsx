          ...
          {careerList.map((career, idx) => (
            <RevealOnScroll key={career.id} delay={idx * 0.05}>
              <div
                ...
                className="group relative h-full rounded-2xl border border-border/80 bg-card p-6 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:border-primary/40 cursor-pointer flex flex-col justify-between"
              >
                {/* Subtle hover gradient glow */}
                <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-r from-primary/20 via-accent/20 to-secondary/20 opacity-0 blur transition duration-300 group-hover:opacity-100 -z-10" />
